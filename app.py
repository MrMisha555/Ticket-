...
from create import *
from bot import bot
from activity_config import BOT_ACTIVITY

# Загружаем переменные окружения и проверяем наличие .env
load_dotenv()
if not os.path.exists(".env"):
    with open(".env", "w") as f:
        f.write("TOKEN=\n")
    print("Файл .env создан. Добавьте в него ваш токен!")
    sys.exit()
TOKEN = os.getenv("TOKEN")
if not TOKEN:
    sys.exit("Ошибка: Токен не найден в .env!")

# Создаем объект бота и объект для slash-команд
bot = DiscordBot()  # или другой способ инициализации бота
